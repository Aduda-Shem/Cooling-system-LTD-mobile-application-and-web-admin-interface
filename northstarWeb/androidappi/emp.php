...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!preg_match($naming, $fname)) {
        ...
        if (date(strtotime($date)) < (strtotime($date_added))) {
            ...
            $select = "SELECT email FROM employee WHERE email='$email'";
            ...
            if (mysqli_num_rows($query) > 0) {
                ...
                $select = "SELECT contact FROM employee WHERE contact='$contact'";
                $query = mysqli_query($lib, $select);
                if (mysqli_num_rows($query) > 0) {
                    $response["success"] = 0;
                    ...
                    fclose($fp);
                    $fine = $hits[0];
                    $mail = $fname . '@staf-' . $fine . '-' . $year . '.northstar.ac.ke';
                    $sql = "INSERT INTO employee(id,fname,lname,gender,contact,email,password,date_added,role,field,address)
