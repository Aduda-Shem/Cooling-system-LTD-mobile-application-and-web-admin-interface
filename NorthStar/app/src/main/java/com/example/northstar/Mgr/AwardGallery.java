...
public class AwardGallery extends AppCompatActivity implements DatePickerDialog.OnDateSetListener, View.OnTouchListener {
    GridView gridView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_award_gallery);
        ...
        gridView.setOnItemClickListener((parent, view, position, id) -> {
            ...
            showDialogImage(AwardGallery.this, position);
        });
        btn.setOnClickListener(v -> {
            AlertDialog.Builder builder = new AlertDialog.Builder(AwardGallery.this);
            builder.setTitle("Add new Award");
            Rect reco = new Rect();
            Window win = AwardGallery.this.getWindow();
            win.getDecorView().getWindowVisibleDisplayFrame(reco);
            LayoutInflater inflater = (LayoutInflater) AwardGallery.this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            ...
            Submit.setOnClickListener(vw -> {
                ...
                if (Desc.isEmpty()) {
                    ...
                    StringRequest stringRequesting = new StringRequest(Request.Method.POST, Router.UPLOAD_AWAR_GALA,
                            respon -> {
                                try {
                                    ...
                                    if (statuses == 1) {
                                        Toast.makeText(AwardGallery.this, msg, Toast.LENGTH_LONG).show();
                                        startActivity(new Intent(getApplicationContext(), AwardGallery.class));

                                    } else if (statuses == 0) {
                                        Toast.makeText(AwardGallery.this, msg, Toast.LENGTH_SHORT).show();
                                    }

                                } catch (JSONException e) {
                                    e.printStackTrace();
                                    // Toast.makeText(SuppDas.this, e.toString(), Toast.LENGTH_SHORT).show();
                                    Toast.makeText(AwardGallery.this, "An error occurred", Toast.LENGTH_SHORT).show();
                                }

                            }, error -> {
                        Toast.makeText(AwardGallery.this, "Failed to connect", Toast.LENGTH_SHORT).show();
    ...
    private void showDialogImage(AwardGallery awardGallery, int position) {
        AlertDialog.Builder alert = new AlertDialog.Builder(this);
        Rect rect = new Rect();
        Window window = awardGallery.getWindow();
        window.getDecorView().getWindowVisibleDisplayFrame(rect);
        LayoutInflater layoutInflater = (LayoutInflater) awardGallery.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        ...
        Glide.with(awardGallery).load(suppFind.getImage()).into(check);
        ...
        alert.setPositiveButton("Delete", (dialog, id) -> {
            StringRequest stringRequest = new StringRequest(Request.Method.POST, Router.DELETE_AWARD,
                    response -> {
                        try {
                ...
                response -> {
                    try {
                        ...
                        if (success == 1) {
                            ...
                            suppAda = new AwardAdapter(AwardGallery.this, R.layout.view_image, SubjectList);
